...
error_count=$(echo $get_error_count_plpgsql_check_query | psql -d $db -t) 
if [ $error_count -eq 0 ]
then
...
echo "::set-output name=plpgsql_check_summary::\"$(echo $info_plpgsql_check_query | psql -d $db -t)\""

psql -d $db -c "COPY ($all_fun_plpgsql_check_query) TO '/tmp/all_fun_plpgsql_check.csv' DELIMITER ',' CSV HEADER;"
